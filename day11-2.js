...
const paintRobot = () => {
	...
	let current = { x, y, paint: 1, painted: false };
...
const minX = Math.min(...Array.from(grid.values()).map(i => i.x));
const minY = Math.min(...Array.from(grid.values()).map(i => i.y));
const maxX = Math.max(...Array.from(grid.values()).map(i => i.x)) + Math.abs(minX);
const maxY = Math.max(...Array.from(grid.values()).map(i => i.y)) + Math.abs(minY);

const result = new Array(maxY + Math.abs(minY)).fill('')
let i = 0;

for (let y = minY; y < maxY; y++) {
	for (let x = minX; x < maxX; x++) {
		const paint = grid.get(`x${(x)}y${(y)}`);
		result[i] += (paint && paint.paint == 1) ? '■' : ' ';
	}
	i++;
}
