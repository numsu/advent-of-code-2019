		...
		for (const dir of Object.values(Direction)) {
			...
			if (!grid.has(positionKey)) {
				...
				if (result.value > 0) {
					...
					moved = true;
					grid.set(positionKey, Block.SPACE);
				} else {
					walledDroids.push(cloned);
					grid.set(positionKey, Block.WALL);
				}
			}
		}
	}
...
const oxygenTankPosition = finishedDroids[0].pos;
let oxygenPositions = new Set();
oxygenPositions.add(key(oxygenTankPosition));
let minutes = 0;
while(true) {
	let moved = false;
	const newGeneration = new Set();
	for (const oxygen of Array.from(oxygenPositions.keys())) {
		for (const dir of Object.values(Direction)) {
			const newPosition = unkey(oxygen).map((a, i) => a + dir.pos[i]);
			const positionKey = key(newPosition);
			const gridPosition = grid.get(positionKey);
			if (gridPosition == Block.SPACE) {
				grid.set(positionKey, Block.OXYGEN);
				newGeneration.add(positionKey);
				moved = true;
			}
		}
	}
	...
	minutes++;
	oxygenPositions = newGeneration;
}

console.log(`The amount of minutes the area takes to fill up with oxygen is ${minutes}`);
